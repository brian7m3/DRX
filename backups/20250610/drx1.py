...
def play_sound(filename, interruptible=False, loop=False, restartable=False):
    ...
    print(f"Playing sound on device: {SOUND_DEVICE} - file: {filename} (Interruptible={interruptible}, Loop={loop}, Restartable={restartable})")
    try:
        subprocess.run(
...
def play_sudo_random_section(base, end, interval, last_played_dict, current_track_dict):
    # For now, implement same as randomized section
    play_randomized_section(base, end, interval, last_played_dict, current_track_dict)

def process_command(command):
    # Expected commands: Pxxxx where xxxx is a number
    if not command.startswith(config['Direct']['prefix']):
        return
    ...
    # Direct play of file if enabled and exact file exists
...
def status_screen(stdscr):
    global serial_buffer, currently_playing
    curses.curs_set(0)
    stdscr.nodelay(True)
    while True:
        stdscr.erase()
        ...
        if 'Random' in config:
            ...
            for b, e, t in zip(bases, ends, intervals):
                ...
                stdscr.addstr(y, 0, f"Base {b}: Track={os.path.basename(current) if current != 'N/A' else 'N/A'} Remaining={remaining:.1f}s")
                y += 1
        ...
        clean_buffer = serial_buffer.replace('\0', '')
        stdscr.addstr(y + 1, 0, f"Serial Buffer: {clean_buffer.strip()}")
        stdscr.addstr(y + 2, 0, f"Currently Playing: {currently_playing if currently_playing else 'None'}")
        stdscr.addstr(y + 4, 0, "Press 'q' to quit.")
        stdscr.refresh()
...
def process_serial_commands():
    ...
    import re
    while True:
        match = re.search(rf"{config['Direct']['prefix']}\d{{4}}", serial_buffer)
        if match:
...
def main():
    ...
    command_thread = threading.Thread(target=process_serial_commands, daemon=True)
