...
def play_randomized_section(base, end, interval, last_played_dict, current_track_dict, interruptible=False):
    """Random mode: select any file in base+1..end, random, but base is just trigger."""
    try:
...
def play_rotating_section(base, end, interval, last_played_dict, current_track_dict, interruptible=False):
    """Rotation mode: rotates through base+1..end. base is only the trigger."""
    try:
        current_time = time.time()
        last_played = last_played_dict.get(base, 0)
        # The rotation set is base+1..end inclusive
        valid_tracks = list(range(base + 1, end + 1))
        # Init: current_track_num is last played, or the first in valid_tracks
        current_track_num = current_track_dict.get(base, valid_tracks[0])
        if current_time - last_played >= interval:
            idx = valid_tracks.index(current_track_num) if current_track_num in valid_tracks else 0
            next_idx = (idx + 1) % len(valid_tracks)
            next_track_num = valid_tracks[next_idx]
...
def play_sudo_random_section(base, end, interval, last_interval_dict, interval_track_dict, played_in_cycle_dict, interruptible=False, repeat=False, pausing=False):
    """SudoRandom mode: picks randomly from base+1..end, cycles all before repeating."""
...
def play_direct_track(code_str, interruptible=False, repeat=False, pausing=False):
    """Directly play a specific track if it exists."""
...
def process_command(command):
    ...
    try:
        ...
        repeat = (suffix == "R")
        pausing = (suffix == "P")
        # Check group logic for Random, Rotation, SudoRandom
        group_found = False
        # RANDOM section
        if 'Random' in config:
        ...
        if 'SudoRandom' in config:
            ...
            for b, e, t in zip(bases, ends, intervals):
                if code == b:
                    ...
                    play_direct_track(code_str, interruptible, repeat, pausing)
                    return
        # If not handled above, fallback to Direct section
...
def write_state():
    # Add as much state as you want to show on the web dashboard!
    state = {
        ...
        # Optionally: 'cos_active': is_cos_active(),
    }
    try:
...
def reload_config():
    ...
    try:
        ...
        COS_DEBOUNCE_TIME = float(config['GPIO']['cos_debounce_time'])
        MAX_COS_INTERRUPTIONS = int(config['GPIO']['max_cos_interruptions'])
        # Optionally, re-setup GPIO if pins changed
        try:
            GPIO.setmode(GPIO.BCM)
            GPIO.setup(REMOTE_BUSY_PIN, GPIO.OUT)
            GPIO.output(REMOTE_BUSY_PIN, not REMOTE_BUSY_ACTIVE_LEVEL)
    ...
    if os.path.exists(WEBCMD_FILE):
        try:
            ...
            if cmd.get("type") == "play":
                ...
                # If input looks like a filename, play it directly!
