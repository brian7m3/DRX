...
<body>
    <h1>DRX Status Dashboard (AJAX Polling)</h1>
    <ul>
        <li><b>Currently Playing:</b> <span id="playing"></span></li>
        <li><b>Status:</b> <span id="status"></span></li>
        <li><b>COS Active:</b> <span id="cos"></span></li>
        <li><b>Serial Port:</b> <span id="serialport"></span></li>
        <li><b>Sound Card:</b> <span id="soundcard"></span></li>
        <li><b>Timestamp:</b> <span id="timestamp"></span></li>
    </ul>
    <script>
        function updateStatus() {
            ...
            .then(data => {
                ...
                document.getElementById('cos').textContent        = data.cos;
                document.getElementById('serialport').textContent = data.serialport;
                document.getElementById('soundcard').textContent  = data.soundcard;
...
@app.route('/status')
def status():
    # Build status dict using your current globals
    return jsonify({
        "playing": currently_playing if currently_playing else "None",
        "status": playback_status if playback_status else ("Idle" if not currently_playing else "Busy"),
        "cos": "YES" if is_cos_active() else "NO",
        "serialport": config['Serial']['port'] if not serial_port_missing else "NOT FOUND",
        "soundcard": SOUND_DEVICE if not sound_card_missing else "NOT FOUND",
