# -*- coding: utf-8 -*-
# DRX logic is 100% untouched from your last "working" script.
# The web dashboard is bolted on, and Play Input calls process_command with the P prefix, matching your DRX serial logic.
# Play Dropdown is direct file play only.
# Web/curses "flash/ansi" output is filtered out. Play Input and Dropdown both work.
        ...
        try:
            if serial_port and serial_port.in_waiting:
                ...
                try:
                    ...
                    # Filter out ANSI and non-printable chars
...
# --- Web UI bolt-on below. DRX core logic above is 100% untouched. ---
app = Flask(__name__)
...
<div id="main-card">
    ...
    <div class="card-section">
        ...
        <form method="POST" action="{{ url_for('play_track') }}">
            ...
            <input name="track_input" type="text" placeholder="e.g. 0001 or 0001I or P0001I" size="20">
...
def play_track():
    ...
    # Dropdown: direct file play (no DRX logic)
    if track_dropdown:
        if play_specific_track(track_dropdown):
            web_log.append(f"{datetime.datetime.now()}: Played track {track_dropdown} from web (file)")
        else:
            web_log.append(f"{datetime.datetime.now()}: Play failed for track {track_dropdown} (not found)")
        return redirect(url_for('dashboard'))
    # Input: DRX logic (add "P" if needed)
    if track_input:
        cmd = track_input
        if not cmd.upper().startswith('P') and re.match(r'^\d{4}[IRP]?$', cmd.upper()):
            cmd = 'P' + cmd
        process_command(cmd)
        web_log.append(f"{datetime.datetime.now()}: Played input {cmd} from web (DRX logic)")
    return redirect(url_for('dashboard'))
...
def run_web_server():
    ...
    try:
        config.read(config_file_path)
        port = int(config['WebAuth'].get('port', '8080').strip())
    except Exception:
        pass
    import logging
