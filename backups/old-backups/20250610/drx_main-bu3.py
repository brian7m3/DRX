...
def play_rotating_section(base, end, interval, last_played_dict, current_track_dict, interruptible=False):
    ...
    try:
        ...
        # The first track to play if never played before:
        if base not in current_track_dict:
            current_track_num = valid_tracks[0]
        else:
            current_track_num = current_track_dict[base]

        if last_played == 0:  # Never played before: play first track
