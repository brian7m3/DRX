...
# --- Utility Functions ---
# Load existing activity minutes on startup
def load_activity_minutes():
    global cos_today_seconds, cos_today_minutes, last_written_minutes
    # This is the correct syntax when using "from datetime import datetime"
            ...
            with open(ACTIVITY_FILE, 'r') as f:
                for line in f:
                    if line.startswith(today):
                        ...
                        if len(parts) >= 2:
                            try:
                                # Extract just the number from the value
                                minutes_str = parts[1].strip()
                                # Remove any text like "minutes" from the value
                                minutes_str = minutes_str.split()[0] if ' ' in minutes_str else minutes_str
                                cos_today_minutes = int(minutes_str)
                                cos_today_seconds = cos_today_minutes * 60
                                last_written_minutes = cos_today_minutes
                                print(f"Loaded previous activity: {cos_today_minutes} minutes")
                            except ValueError:
                                print(f"Invalid minute count in activity file: {parts[1]}")
                        break
        except Exception as e:
...
def serial_read_loop():
    ...
    serial_port = None
    reconnect_delay = 5  # Start with 5 seconds delay for reconnection attempts
    ...
    while True:
        ...
        # Now we have a connection, try to read data
        try:
            if serial_port and serial_port.is_open and serial_port.in_waiting > 0:
                ...
                # No data waiting but port is open - nothing to do
                pass
        ...
        # Always sleep a bit to prevent CPU spinning
        time.sleep(0.05)

def process_serial_commands():
    """
    Continuously scan the serial_buffer for valid command patterns, including standalone A1 command.
    """
...
def speak_activity_minutes_for_previous_day():
    ...
    try:
        ...
        # Keep looping until we get through a full debounce period without COS becoming active
        while True:
            # Step 1: Wait for COS to become inactive if it isn't already
            if is_cos_active():
            ...
            # Step 2: Start the debounce timer
            debounce_start = time.time()
            debounce_complete = False
            
            # Step 3: Monitor COS during the debounce period
            while time.time() - debounce_start < COS_DEBOUNCE_TIME:
                if is_cos_active():
                    debug_log(f"A1 COMMAND: COS became active again during debounce period, restarting wait process")
                    break  # Break out of this loop and restart the outer loop
                time.sleep(0.1)
            
            # Step 4: Check if we made it through the full debounce period
            if time.time() - debounce_start >= COS_DEBOUNCE_TIME:
                ...
                break  # Exit the outer loop and proceed with playback
        
        # Debounce period completed successfully, now play the activity report
        debug_log("A1 COMMAND: Starting activity report playback")
        playback_status = "Activity Report: Playing"
        currently_playing = "Activity Report"
        currently_playing_info = "Speaking previous day's activity"
        currently_playing_info_timestamp = time.time()
        write_state()
        ...
        log_recent(f"Activity Report: Previous day ({prev_date}) had {minutes} minutes of activity")
        
        # Play each wav file in order without interruption
        for wav in wavs:
...
def main():
    ...
    # --- Load activity minutes ---
    load_activity_minutes()  # PLACE IT HERE
    ...
    # --- Load persistent state before starting threads ---
    load_state()
