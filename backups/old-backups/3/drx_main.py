...
def process_command(command):
    ...
    ...
    debug_log(f"process_command called with: {command}")
    try:
        playback_interrupt.set()
        time.sleep(0.1)
        code_str, suffix, alt_code = parse_serial_command(command.strip())
        # print(f"parse_serial_command returned: code_str={code_str}, suffix={suffix}, alt_code={alt_code}")
        if code_str is None:
            return

        # Robust COS-i logic!
        if suffix == 'i' and alt_code is not None:
            # print("COS-i logic triggered!")
            ...
            def cos_i_sequence():
                ...
                try:
                    ...
                    while proc.poll() is None:
                        if is_cos_active():
                            # print("[COS-i] COS ACTIVE! Stopping 1st file.")
                    ...
                    if not cos_triggered:
                        # ... 1st file ended before COS, done
                        return
                    log_recent("[COS-i] Interrupt triggered, playing 2nd file immediately")
                    # (No while loop, don't wait for COS to go inactive)
        ...
        # --- Everything else is your original logic below ---
