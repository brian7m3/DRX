...
def process_command(command):
    ...
    try:
        ...
        if is_alt and bases:
            ...
            # Ensure persistent dicts for this key
            if key not in alternate_series_track_pointers:
                alternate_series_track_pointers[key] = {base: 1 for base in bases}
            if key not in alternate_series_last_played:
                alternate_series_last_played[key] = {base: 0 for base in bases}
            ...
            if typ == "Rotation":
                track_index = track_pointers.get(base, 1)
                track_num = base + (track_index if isinstance(track_index, int) else 1)
                base_filename_prefix = f"{track_num:04d}"
                all_files = sorted(
                    f for f in os.listdir(SOUND_DIRECTORY)
                    if (f.startswith(f"{base_filename_prefix}-") or f.startswith(f"{base_filename_prefix}.") or f.startswith(base_filename_prefix))
                    and f.endswith(SOUND_FILE_EXTENSION)
                )
                debug_log(f"All files for rotation track {track_num}: {all_files}")
                if not all_files:
                    debug_log(f"No files found for rotation track {track_num} in {SOUND_DIRECTORY}")
                    pointer = (pointer + 1) % len(bases)
                    alternate_series_pointers[key] = pointer
                    return
                filename = os.path.join(SOUND_DIRECTORY, all_files[0])

                # Set timer BEFORE starting playback thread
                if last == 0:
                    debug_log(f"First play for base {base}. Setting timer and playing track {track_num}.")
                    last_played[base] = now
                    threading.Thread(target=play_sound, args=(filename, interruptible, repeat, pausing), kwargs={'source': "serial/auto"}).start()
                elif now - last >= interval:
                    debug_log(f"Interval expired for base {base}. Advancing to next track.")
                    if track_num < end:
                        track_pointers[base] = track_index + 1
                    else:
                        track_pointers[base] = 1
                    last_played[base] = now
                    threading.Thread(target=play_sound, args=(filename, interruptible, repeat, pausing), kwargs={'source': "serial/auto"}).start()
                else:
                    debug_log(f"Interval NOT expired for base {base}. Replaying current track {track_num}.")
                    threading.Thread(target=play_sound, args=(filename, interruptible, repeat, pausing), kwargs={'source': "serial/auto"}).start()
                ...
                # Ensure played is a set (may be loaded as list from JSON)
            ...
            # Save state back to the global dicts
        ...
        if suffix == 'i' and alt_code is not None:
            ...
            def cos_i_sequence():
                ...
                try:
                    currently_playing = os.path.basename(filename1)
                    currently_playing_info = f"[COS-i] Step 1: Playing {filename1} (COS interruptible)"
                    currently_playing_info_timestamp = time.time()
                    playing_end_time = currently_playing_info_timestamp + 5
                    GPIO.output(REMOTE_BUSY_PIN, REMOTE_BUSY_ACTIVE_LEVEL)
                    import subprocess
                    ...
                    proc = subprocess.Popen(
                        ['aplay', '-D', SOUND_DEVICE, filename1],
                        stdout=subprocess.DEVNULL,
                        stderr=subprocess.DEVNULL
                    )
                    cos_triggered = False
...
def write_state():
    ...
    for key, last_played_dict in alternate_series_last_played.items():
        ...
        for base in bases:
            ...
            remaining = max(0, interval - (now - last)) if interval else 0
            track_val = track_pointers.get(base, 1)
            if typ == 'Rotation':
                # track_val is an int offset for Rotation
                current_num = base + (track_val if isinstance(track_val, int) else 1)
                track_display = str(current_num)
            else:
                # Random/SudoRandom: track_val is a filename (str) or None
                track_display = os.path.basename(track_val) if track_val and isinstance(track_val, str) else "N/A"
            alt_bases_lines.append(
                f"Series {key} Base {base} Type {typ} | End {end} Interval {interval//60 if interval else 0}m: Track={track_display} Remaining={remaining:.1f}s"
            )

    # Always reset drx_start_time on process start
