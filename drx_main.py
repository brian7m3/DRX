...
Direct = {
    "enabled": True,   # use boolean, not string 'true'
    "prefix": "P"
}
last_written_minutes = -1
DIRECT_ENABLED = True
...
def log_error(msg):
    """
    Log an error message to drx_error.log, prepending the newest entry at the top.
    """
    try:
        timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        formatted_msg = f"[{timestamp}] {msg}"
        # Prepend: read existing, write new entry first
        ...
        with open(log_file_path, 'w', encoding='utf-8') as f:
            f.write(formatted_msg + '\n' + existing_content)
        # Set permissions to rwxrwxrwx (optional)
        try:
...
def log_exception(context: str):
    """
    Log a caught exception with traceback to drx_error.log, prepending the newest entry at the top.
    """
    timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
...
def log_recent(entry):
    """
    Append a timestamped entry to drx_webconsole.log and trim if file exceeds 500KB.
    """
...
# Optional: check for required programs and log if missing
...
def monitor_cos():
    ...
    while True:
        ...
        if curr_state and not prev_state:
            last_cos_active_time = time.time()  # COS just became active
...
def play_sound(
    ...
    try:
        if wait_for_cos:
            ...
            # Wait for channel to clear (debounce, as in speak_activity_minutes_for_previous_day)
            while True:
                if is_cos_active():
            ...
            # Now play the sound normally (like the default branch)
            proc = None
            try:
...
def play_randomized_section(
    ...
    try:
        ...
        play_sound(new_track, interruptible, pausing, repeating, wait_for_cos=wait_for_cos)
    except Exception:
...
def play_rotating_section(
    ...
    try:
        ...
        play_sound(next_track, interruptible, pausing, repeat, wait_for_cos=wait_for_cos)
...
def play_sudo_random_section(
    ...
    play_sound(file_to_play, interruptible, pausing, repeat, wait_for_cos=wait_for_cos)
        ...
        if is_alt and bases:
            ...
            for step in range(len(bases)):
                ...
                if base_filename:
                    ...
                    # cosi_interrupt_code support removed, as per-base suffix is now used
                    play_sound(
    ...
    while True:
        try:
            ...
            if y < max_y - 2:
                ...
                remote_state = is_remote_device_active()
...
def main():
    ...
    try:
        ...
        if is_terminal():
            ...
            #Call the fallback routine or just keep threads running:
            fallback_command_prompt()
            while True:
                time.sleep(1)
    except KeyboardInterrupt:
