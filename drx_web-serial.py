...
SERIAL_BAUDRATE = int(config.get('Serial', 'baudrate', fallback='57600'))
    ...
    <div style="margin: 14px 0;">
      {% for category, msg in messages %}
        <div class="flash-message flash-{{ category }}" style="
            ...
            border-radius: 6px;
            margin-bottom: 4px;
            {% if category == 'error' %}background: #ffeaea; color: #b71c1c; border: 1px solid #e57373;
            {% elif category == 'success' %}background: #e4fbe7; color: #388e3c; border: 1px solid #81c784;
            {% endif %}
        ">
          {{ msg }}
        </div>
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    <!-- Serial Data Entry Box -->
    <form method="POST" action="{{ url_for('send_serial_data') }}" id="serial-data-form" style="margin-top: 12px; display: flex; flex-direction: row; align-items: stretch; max-width: 420px;">
        <input type="text" id="serial-data-input" name="serial_data" placeholder="Enter serial command" style="flex: 1 1 0; font-size: 1.1em; padding: 7px 10px; border-radius: 6px 0 0 6px;">
        <button type="submit" style="border-radius: 0 6px 6px 0; font-size: 1.1em; padding: 7px 18px;">Enter</button>
    </form>
</div>
<script>
document.addEventListener('DOMContentLoaded', function() {
    var serialForm = document.getElementById('serial-data-form');
    var serialInput = document.getElementById('serial-data-input');
...
def send_serial_data():
    ...
    if serial_data:
        try:
            # Replace <ESC> with ASCII 27
            serial_data = re.sub(r'(?i)<esc>', chr(27), serial_data)
            # Write command for DRX to pick up
            webcmd = {
                "type": "play",
                "input": serial_data,
                "char_delay_ms": 6,
            ...
            with open("/tmp/drx_webcmd.json", "w") as f:
                json.dump(webcmd, f)
            flash("Command sent to DRX!", "success")
        except Exception as e:
            flash(f"Error sending command to DRX: {e}", "error")
    else:
